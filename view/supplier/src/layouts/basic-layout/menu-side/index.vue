...
export default {
  ...
  watch: {
		$route: {
		  handler(n) {
				...
				storage.setItem('chiidLinkSupplier',activeLink);
				let funLink = function (children,index) {
					children.forEach((item)=>{
						if(activeLink == item.path){
						    that.parentCur = index
								storage.setItem('parentLinkSupplier',that.filterSider[index].path);
  ...
  methods: {
    activeMenu(){
      let storage=window.localStorage;
			let activeLink = storage.getItem('parentLinkSupplier');
			let chiidLink = storage.getItem('chiidLinkSupplier');
			if(storage.getItem('isChildrenSupplier') == null || activeLink == this.filterSider[0].path){
				...
				this.$store.commit('store/layout/setChildren', JSON.parse(storage.getItem('isChildrenSupplier')));
				this.filterSider.forEach((item,index)=>{
				  if(activeLink == item.path){
    ...
    handelParentClick(nav,index){
      ...
      if(nav.children){
        ...
        this.$store.commit('store/layout/setChildren', true);
        storage.setItem('isChildrenSupplier',true);
      }else {
        chiidLink = nav.path
        this.$store.commit('store/layout/setChildren', false);
        storage.setItem('isChildrenSupplier',false);
      }
      this.$router.push({
        path:chiidLink
      })
      storage.setItem('parentLinkSupplier',nav.path);
			storage.setItem('chiidLinkSupplier',chiidLink);
    },
