...
function getMenusName () {
    let storage = window.localStorage, menuList = storage.getItem('menuListSupplier'), menuData = []
    try {
...
export default {
    ...
    mutations: {
        getmenusNav (state, menuList) {
            ...
            storage.setItem('menuListSupplier', JSON.stringify(menuList));
        }
    },
    actions: {
        getMenusNavList ({ commit }) {
            return new Promise((resolve, reject) => {
                menusApi().then(async res => {
                    ...
                    let storage = window.localStorage;
                    storage.setItem('menuListSupplier', JSON.stringify(res.data.menus));
                }).catch(res => {
                    reject(res);
                })
            })
