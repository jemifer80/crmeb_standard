				...
				<view id="past0">
					...
					<view class='wrapper'>
						<view class="money acea-row row-middle">
							<image src="static/my-point.png" mode="" v-if="parseFloat(storeInfo.integral)"></image>
							<text class="num font-color" v-if="parseFloat(storeInfo.integral)">{{storeInfo.integral || 0}}</text>
							<text class="plus" v-if="parseFloat(storeInfo.integral) && parseFloat(storeInfo.price)">+</text>
							<view class="num font-color" v-if="parseFloat(storeInfo.price)"><text class="labels">¥</text>{{storeInfo.price || 0}}</view>
						</view>
						...
						<view class='label acea-row row-middle'>
							...
							<view class='stock'>限量:
								{{ storeInfo.quota_show}}{{storeInfo.unit_name || ''}}
							</view>
							<view class='stock'>已兑换：{{storeInfo.sales}}次
							</view>
						</view>
					</view>
		...
		methods: {
			...
			DefaultSelect: function() {
				...
				if (productSelect && productAttr.length) {
					...
					self.$set(self.attribute.productSelect, "integral", self.storeInfo.integral);
