      ...
      <Row type="flex" v-show="currentTab === '1'">
          ...
          <div class="acea-row row-middle">
            <div class="labelInput acea-row row-between-wrapper" @click="openStoreLabel">
              <div v-if="storeDataLabel.length">
                <Tag closable v-for="(item, index) in storeDataLabel" :key="index" @on-close="closeStoreLabel(item)">{{
                    item.label_name
                }}</Tag>
              </div>
        ...
        <FormItem label="供应商：" prop="supplier_id" v-if="formValidate.product_type == 0">
          <Select v-model="formValidate.supplier_id" clearable filterable v-width="'50%'" placeholder="请选择供应商">
            <Option v-for="(item, index) in supplierList" :value="item.id" :key="index">{{ item.supplier_name }}
            </Option>
          </Select>
        </FormItem>
        </Col>
        ...
        <FormItem label="选择规格：" prop="">
          <div class="acea-row row-middle">
            <Select v-model="formValidate.selectRule" style="width: 23%">
              <Option v-for="(item, index) in ruleList" :value="item.rule_name" :key="index">{{ item.rule_name }}
              </Option>
            </Select>
      ...
      <Row v-show="currentTab === '5'">
        ...
        <FormItem label="赠送优惠券：">
          <div v-if="couponName.length" class="mb20">
            <Tag closable v-for="(item, index) in couponName" :key="index" @on-close="handleClose(item)">{{ item.title
            }}</Tag>
          </div>
          ...
          <div class="labelInput acea-row row-between-wrapper" @click="openLabel">
            <div>
              <div v-if="dataLabel.length">
                <Tag closable v-for="(item, index) in dataLabel" @on-close="closeLabel(item)">{{ item.label_name }}
                </Tag>
              </div>
        ...
        <FormItem label="选择优品推荐商品：">
          <div class="acea-row">
            <div class="pictrue" v-for="(item, index) in goodsData" :key="index">
      ...
      <Row v-show="currentTab === '6'">
        ...
        <FormItem label="服务保障：">
          <CheckboxGroup v-model="formValidate.ensure_id" class="checkAlls">
            <Checkbox :label="item.id" v-for="(item, index) in ensureData">{{
                item.name
            }}</Checkbox>
          </CheckboxGroup>
        </FormItem>
