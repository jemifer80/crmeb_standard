...
<!--                            <Option value="">全部</Option>-->
        ...
        <Card :bordered="false" dis-hover class="ivu-mt">
            <Button v-auth="['setting-system_admin-add']" type="primary" @click="add">添加业务员</Button>
        ...
        <!-- 业务员用户数 -->
        <template slot-scope="{ row, index }" slot="salesman_users">
          <div class="acea-row">
            <div v-text="row.salesman_users"></div>
          </div>
        </template>
       <!-- 业务员订单数 -->
        <template slot-scope="{ row, index }" slot="salesman_orders">
          <div class="acea-row">
            <div v-text="row.salesman_orders"></div>
          </div>
        </template>
        <!-- 业务员订单金额 -->
        <template slot-scope="{ row, index }" slot="salesman_amount">
          <div class="acea-row">
            <div v-text="row.salesman_amount"></div>
          </div>
        </template>
        <!-- 业务员提成 -->
        <template slot-scope="{ row, index }" slot="salesman_award">
          <div class="acea-row">
            <div v-text="row.salesman_award"></div>
          </div>
        </template>  
...
<script>
    ...
    import {adminSalesmanListApi,adminSalesmanFromApi,adminSalesmanEditFromApi,setShowApi } from '@/api/systemAdmin';
    ...
    export default {
        ...
        data () {
            return {
                ...
                columns1: [
                    ...
                    {
                        title: '订单数',
                        slot: 'salesman_orders',
                        minWidth: 150,
                    },
                    {
                        title: '销售金额',
                        slot: 'salesman_amount',
                        minWidth: 150,
                    }, 
                    {
                        title: '提成',
        ...
        methods: {
            ...
            getList () {
                ...
                adminSalesmanListApi(this.formValidate).then(async res => {
            ...
            // 添加表单
            add () {
                adminSalesmanFromApi().then(async res => {
            ...
            // 编辑
            edit (row) {
                adminSalesmanEditFromApi(row.id).then(async res => {
            ...
            onchangePaymentTime(e) {
               ...
               this.formValidate.payment_time = this.paymentTimeVal.join("-");
            },             
            // 表格搜索
            userSearchs () {
