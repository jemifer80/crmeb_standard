...
class UserController
{
    ...
    {
        [$avatar, $merchant_name, $extend_info] = $request->postMore([
            ['avatar', ''],
            ['merchant_name', ''],
            ['extend_info', []]
        ], true);
        if (!$avatar && $merchant_name == '') {
            return app('json')->fail('请输入商户名称或者选择头像');
        }
        $uid = (int)$request->uid();
		$this->services->saveExtendForm($uid, $extend_info, ['avatar' => $avatar, 'merchant_name' => $merchant_name], true);
