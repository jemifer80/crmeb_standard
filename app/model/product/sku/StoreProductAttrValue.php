...
class StoreProductAttrValue extends BaseModel
{
    ...
    {
        return $this->hasOne(ProductAttrValue::class, 'unique', 'unique')->bind([
            'branch_sales' => 'sales',
            'branch_stock' => 'stock'
        ]);;
    }
    ...
    {
        return $this->hasOne(StoreIntegral::class, 'id', 'product_id')->where('is_show', 1)->where('is_del', 0);
    }
    ...
    {
        if (is_array($value)) {
            if ($value) $query->whereIn('suk', $value);
        } else {
            if ($value !== '') $query->where('suk', $value);
        }
    }
