...
class PayNotifyServices
{
     ...
     * @return bool
     */
    public function wechatIntegral(string $order_id = null, string $trade_no = null)
    {
        try {
            /** @var StoreIntegralOrderServices $integralOrderservices */
            $integralOrderservices = app()->make(StoreIntegralOrderServices::class);
            $orderInfo = $integralOrderservices->getOne(['order_id' => $order_id]);
            if (!$orderInfo) {
                $orderInfo = $integralOrderservices->getOne(['unique' => $order_id]);
                if (!$orderInfo) return true;
            }
            if ($orderInfo->paid) return true;
            return $integralOrderservices->paySuccess($orderInfo->toArray(), PayServices::WEIXIN_PAY, ['trade_no' => $trade_no]);
        } catch (\Exception $e) {
            return false;
        }
    }
     ...
     * @return bool
     */
    public function aliyunIntegral(string $order_id = null, string $trade_no = null)
    {
        if (!$order_id || !$trade_no) {
            return false;
        }
        try {
            ...
            if (!$orderInfo) return true;
            if ($orderInfo->paid) return true;
            return $integralOrderservices->paySuccess($orderInfo->toArray(), PayServices::ALIAPY_PAY, ['trade_no' => $trade_no]);
        } catch (\Throwable $e) {
            return false;
        }
    }
